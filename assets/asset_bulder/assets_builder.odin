...
main :: proc() {
	...
	{
		file_infos := dir_path_to_file_infos("../shaders")
		for fi in file_infos {
		...
		{
		file_infos := dir_path_to_file_infos("../fonts")
		for fi in file_infos {
		...
		if embed {
			if include_ext_dat {
				for p in paths {
					...
					fmt.fprintf(f, "\t\t.%s = {{ path = \"%s\", path_hash = %v, data = #load(\"%s\"),info = #load(\"%s\",cstring) or_else #load(\"textures/default.txt\", cstring), }},\n", asset_name(p), p, hash.murmur64a(transmute([]byte)(p)), p,p2)
